#!/usr/bin/env python3
# asymmetric_grid.py — Grid showing asymmetric entangled state (biased 00/11)
# Author: Pito Salas and Claude Code
# Open Source Under MIT license

import math
import time

import streamlit as st

from sim.runner import run_trials_asymmetric
from viz import registry
from viz.two_qubit_grid import animate_two_qubit_grid, build_two_qubit_grid_html

# θ = π/3 gives cos²(π/6) = 75% chance of |00⟩, sin²(π/6) = 25% chance of |11⟩
THETA = math.pi / 3


def render_step_asymmetric(args: list[str], step: int, key: str, placeholder) -> bool:
    return animate_two_qubit_grid(
        lambda: run_trials_asymmetric(THETA, 1), args, step, key, placeholder
    )


def render(args: list[str], placeholder=None) -> None:
    """Blocking render — used as fallback."""
    n = int(args[0]) if args else 16
    results: list[str | None] = [None] * n
    if placeholder is None:
        placeholder = st.empty()

    for i in range(n):
        html = build_two_qubit_grid_html(results[:i + 1], i + 1)
        placeholder.markdown(html, unsafe_allow_html=True)
        time.sleep(0.33)
        single = run_trials_asymmetric(THETA, 1)
        results[i] = next(k for k, v in single.items() if v > 0)
        html = build_two_qubit_grid_html(results[:i + 1], i + 1)
        placeholder.markdown(html, unsafe_allow_html=True)
        time.sleep(0.33)


registry.register("asymmetric-grid", render)
registry.register_step("asymmetric-grid", render_step_asymmetric)
